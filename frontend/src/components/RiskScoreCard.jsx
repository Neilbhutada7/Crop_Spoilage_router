export default function RiskScoreCard({ riskData }) {
  ...
  return (
    <div className="card" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ marginBottom: '1.25rem', fontSize: '1.125rem', fontWeight: '600' }}>AI Spoilage Risk</div>
      ...
      <div className="grid grid-cols-3" style={{ width: '100%', borderTop: '1px solid var(--border)', paddingTop: '1rem', gap: '0.5rem' }}>
        <div>
          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Temp</div>
        ...
        <div>
          ...
          <div style={{ fontWeight: '600' }}>{riskData.days_since_harvest} days</div>
        </div>
      </div>
      ...
      {riskData.risk_factors && riskData.risk_factors.length > 0 && (
        <div style={{ width: '100%', marginTop: '1.5rem', borderTop: '1px solid var(--border)', paddingTop: '1.25rem', textAlign: 'left' }}>
          ...
          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.4' }}>
            <span style={{ fontWeight: '600', color: 'var(--text)' }}>{riskData.risk_label} risk ({Math.round(riskData.risk_score)}/100)</span> — time since harvest and local conditions are driving factors. Point values below represent relative impact against ideal baseline conditions.
          </div>
          ...
          <div style={{ marginTop: '1.5rem', padding: '0.75rem', backgroundColor: 'var(--bg-card)', borderRadius: '6px', fontSize: '0.75rem', color: 'var(--text-muted)', border: '1px solid var(--border)', lineHeight: '1.4' }}>
            <div style={{ fontWeight: '600', marginBottom: '0.25rem', color: 'var(--text)' }}>Live Open-Meteo data · model xgb-v1</div>
            Score is produced by an XGBoost gradient-boosted trees (XGBRegressor), trained on 3,000 real-world samples, then evaluated on a held-out 20% split. 
